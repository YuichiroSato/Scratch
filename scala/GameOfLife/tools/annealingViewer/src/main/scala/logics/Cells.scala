...
class Cells(val xSize: Int, val ySize: Int) {
  ...
  def overlay(scattering: Scattering, x: Int, y: Int): Unit = {
    val ux = cellX(x - 1)
    val bx = cellX(x + 1)
    val cx = cellX(x)
    val cy = cellY(y)
  ...
  def allocate(scattering: Scattering, x: Int, y: Int): Unit = {
    ...
    cells(ux)(ly) = scattering.getCell(0, 0)
    cells(ux)(cy) = scattering.getCell(0, 1)
    cells(ux)(ry) = scattering.getCell(0, 2)
    cells(cx)(ly) = scattering.getCell(1, 0)
    cells(cx)(ry) = scattering.getCell(1, 2)
    cells(bx)(ly) = scattering.getCell(2, 0)
    cells(bx)(cy) = scattering.getCell(2, 1)
    cells(bx)(ry) = scattering.getCell(2, 2)
  }

  def scattering(x: Int, y: Int): Scattering = {
